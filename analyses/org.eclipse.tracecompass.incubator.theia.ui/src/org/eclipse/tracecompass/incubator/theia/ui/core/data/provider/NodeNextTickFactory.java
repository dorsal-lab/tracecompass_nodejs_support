...
public class NodeNextTickFactory implements IDataProviderFactory {
	
	   private static final IDataProviderDescriptor DESCRIPTOR = new DataProviderDescriptor.Builder()
	            .setId(NodeNextTickDataProvider.ID)
	            .setName("Node NextTick  XY Chart") //$NON-NLS-1$
	    ...
	    public @Nullable ITmfTreeDataProvider<? extends ITmfTreeDataModel> createProvider(ITmfTrace trace) {
	        ...
	        if (traces.size() == 1) {
	            return NodeNextTickDataProvider.create(trace);
	        }
	        return TmfTreeXYCompositeDataProvider.create(traces, "Node NextTick XY graph data provider", NodeNextTickDataProvider.ID); //$NON-NLS-1$
	    }
